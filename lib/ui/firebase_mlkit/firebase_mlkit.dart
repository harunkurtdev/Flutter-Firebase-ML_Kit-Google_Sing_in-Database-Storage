...
import 'package:flutter_firebase/ui/firebase_mlkit/api/ml_kit_api.dart';
import 'package:image_picker/image_picker.dart';

import 'package:transparent_image/transparent_image.dart'
    show kTransparentImage;

class FirebaseMLKitExample_Basic extends StatefulWidget {
  const FirebaseMLKitExample_Basic({Key key}) : super(key: key);

  @override
  _FirebaseMLKitExampleState createState() => _FirebaseMLKitExampleState();
}

class _FirebaseMLKitExampleState extends State<FirebaseMLKitExample_Basic> {
   File _imageFile;
  String _mlResult = '<no data>';
  ...
  Future<Null> _faceDetect() async {
    setState(() => this._mlResult = '<no data>');
    if (await _pickImage() == false) {
      return;
    }
    String result = '';
        ...
        child: ListView(
          children: <Widget>[
            this._imageFile == null
                ? Container(
                  child: Image.network("https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTyD9tiJspFs6H9s8yQx19bfB2vA6k6wNEq-lVS3-Rcqlf-hTDo",
                  fit: BoxFit.contain,
                  ),
                  height: 200,
                )
